# Contribuir

1. Lê [docs/COMO-RODAR.md](docs/COMO-RODAR.md) e [docs/PLANEJAMENTO.md](docs/PLANEJAMENTO.md).
2. **Não** commits com secrets OAuth, refresh tokens ou `.env` com credenciais reais.
3. Preferência por alterações **pequenas e focadas**; mantém a app **leve** (questiona dependências novas).
4. `npm run dev` para desenvolvimento; `cargo fmt` / `cargo clippy` no `src-tauri/` antes de PR quando possível.
5. Idioma dos docs de produto: **português**; nomes de código/API em inglês são aceitáveis.

Pull requests: descreve o problema ou objetivo e como testaste no **Windows**.
